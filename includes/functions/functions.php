...
function getusers_by_id($user_id){
    global $conn;
...
function getitems($where,$CatID){
    global $conn;
    $sql = "SELECT * FROM items WHERE $where=? ORDER BY Item_ID  ASC ";
    $getcat = $conn->prepare($sql);
    $getcat->execute([$CatID]);
    $data = $getcat->fetchAll();
    return $data ;
}
